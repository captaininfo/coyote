...
| WIKIDATA_DISAMBIG_MARGIN | 0.05 | Unit 8 margin guard (`wikidata_disambiguation`): a non-prominence candidate must beat prominence #1's cosine by **≥ this** to override it; otherwise prominence #1 wins. `0.0` disables the guard (pure argmax re-rank); higher = overrides harder. Protects already-correct prominence-#1 mappings (Gate 8.2) from marginal cosine flips. **Selection order:** margin picks the winner, then `WIKIDATA_DISAMBIG_THRESHOLD` gates it. Same import-time / restart-to-change semantics as the threshold. |
