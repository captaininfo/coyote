...
Session 3 verification gates A/B/C remain *separately pending*. Do not block this refactor on them — but if Gate B fails (zero meta-class edges), Session 3 itself needs revisiting before this refactor starts.
...
3. **Session 3 gate status — DECISION PENDING (raised 2026-05-27).** The plan text below describes the conservative position (measure Gate B before proceeding). The pragmatic alternative is to skip: Unit 9d's P31 instance-of blocklist directly supersedes Session 3's WIKIMEDIA_META_URIS fix and addresses the same failure mode more durably; any meta-class edges produced during intermediate refactor units get wiped at final verification (data expendable). Justin leaning toward skip; CLAUDE.md and this plan both currently say go/no-go. Resolve before pre-flight 3 actually runs. — *Conservative text:* Confirm Gates A/B/C from the prior Session 3 deploy have actually been measured. If they haven't, run them. Refactor proceeds only on Session 3 known-good or known-bad-but-irrelevant.
...
**Work breakdown:**
- Investigate root cause. Likely candidates: search events skip the NLP pipeline; `analyze_topics` fails on short text; SERP scraping returns no text; staged differently.
- Decide: should search-result-page (SERP) Webpages have topics? Or should they be exempt by design? This is a design question, not just a bug fix.
- If exempt by design: tag them explicitly (`topic_skip_reason = "serp"`) so they don't pollute gate measurements.
- If they should have topics: fix the upstream cause.

**Verification gate (defined only after Unit 1's design decision):** the SERP-topics design decision is itself part of this unit's deliverable. The gate cannot be defined until that decision lands. **Required output of Unit 1, before proceeding to Unit 2:** a documented "expected distribution" of Webpage nodes with zero HAS_TOPIC edges, broken down by category (exempt URLs, empty-scrape URLs, SERP-exempt-by-design, true-no-topic). The gate then becomes: on a fresh browsing replay, the observed distribution matches this documented expectation within tolerance.
...
- API target: `extract_keywords(text: str, doc_embedding: np.ndarray, top_n: int = 20, mmr_lambda: float = 0.6) -> List[Tuple[str, float]]`.
...
- Custom vectorizer per Pekar: `CountVectorizer` with spaCy PoS-aware tokenization, filtering to NOUN / ADJ / PROPN, respecting sentence boundaries.
- **Two spaCy instances required (NOT shared).** The Pekar approach uses `spacy.load("en_core_web_sm", disable=["parser", "ner"])` for speed and because parser/NER outputs are not used by the vectorizer. Coyote's NER step (`text_ner_analysis.py`) needs parser AND NER enabled. These are mutually exclusive configurations — you cannot share one instance. Load a second spaCy instance at the KeyBERT module's initialization, distinct from the NER pipeline's instance. Additional memory cost is ~50 MB; acceptable on Coyote's target hardware but document the tradeoff in CLAUDE.md.
...
**Verification gates:**
- **Gate 3.1 (functional):** on a sample of 20 representative pages, KeyBERT produces 5-20 topics each. No spurious cross-sentence n-grams (e.g., no "supervised learning machine"-style errors). Top-5 phrases pass human spot-check for relevance.
...
- **Gate 3.4 (score sanity):** baseline `Topics.score` distribution set empirically from pre-flight check 2 results (not hardcoded). Gate: no values >1.0 or <0.0; distribution is non-degenerate (not all values clustered at one end — sample IQR should be meaningfully non-zero).
...
**Verification gates:**
- **Gate 5.1:** re-measure post-deploy. Improvement target = +N percentage points over the freshly-measured `coyote-0.4` baseline (N to be set after baseline measurement; do not hardcode 80% until baseline is known).
...
- After KeyBERT (Unit 3) returns phrases, before WikiData mapping:
  ...
  - Drop phrases of length <3 characters.
...
**Verification gates:** each sub-item has its own small gate (e.g., for 9b, no Webpage has more than N HAS_TOPIC edges; for 9c, a sample query distinguishes the three skip reasons; for 9d, after a Wikidata schema change to category labels, mapping still works).
...
- **R1 — Pekar custom vectorizer integration is more subtle than the summary suggests.** Article uses both `tokenizer=` parameter and `_word_ngrams` override. *Mitigation:* pre-flight check 1 builds the proof-of-concept before opening the branch. If POC fails, fall back to KeyBERT default vectorizer with stricter post-extraction filter (Unit 6).
- **R2 — KeyBERT diversity (MMR) doesn't give acceptable top-N on Coyote's content.** *Mitigation:* pre-flight check 2 verifies on real Coyote pages. If unacceptable, revisit BERTopic (rolling back item 2) — branch model makes this a clean rollback.
...
- MMR λ for KeyBERT (Unit 3c): default 0.6, pre-flight 2 may adjust.
...
- `TOPIC_SCORE_THRESHOLD` value (Unit 3e): empirical, set after observing the post-KeyBERT score distribution.
...
- Gate 5.1 numeric target (Unit 5): set after re-measuring the current empty-scrape baseline on `coyote-0.4` HEAD.
...
- **Unit 3c (KeyBERT):** custom vectorizer rejects cross-sentence n-grams (test against the Pekar "supervised learning machine" pattern); MMR with λ=0 returns ranked-by-similarity only; MMR with λ=1 returns max-diversity only.
...
- **CLAUDE.md:** BERTopic → KeyBERT swap; new `Topics.score` semantics; new `Entities.score` semantics; new Webpage node properties from trafilatura; updated `embedding_model` invariant if Unit 10 selects a new model; updated `VECTOR_SIMILARITY_THRESHOLD` baseline if Unit 10 re-baselines; removed dependencies (`bertopic`, `umap-learn`, `hdbscan` if no longer pulled); Known Issues entries closed or updated; new known dependency: two spaCy instances (Unit 3c) noted with memory tradeoff.
