...
- New Step 7.5 in the webpage path: `emb_text = title + "\n\n" + scraped_text` (title prepended for retrieval signal; bare `scraped_text` when no title); `doc_embedding = embed_document(emb_text)`. This one embedding is passed to KeyBERT (Step 8) AND persisted at Step 20.5 (`embedding`, `embedding_text = emb_text`, `embedding_generated_at`). `embedding_text` duplicating `scraped_text` on the node is accepted at MVP (local-first, data-expendable); it dissolves when chunk nodes carry their own text post-MVP.
