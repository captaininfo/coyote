...
3. **Session 3 gate status.** Confirm Gates A/B/C from the prior Session 3 deploy have actually been measured. If they haven't, run them. Refactor proceeds only on Session 3 known-good or known-bad-but-irrelevant.
...
6. **Empty-scrape baseline.** Re-measure the current empty-scrape rate on `coyote-0.4` HEAD across a fresh browsing replay. The 67% figure in CLAUDE.md is pre-trafilatura and not the live baseline. Factor out redirect-URL events from the denominator. Use the result to set Gate 5.1's empirical target.
