...
- Custom vectorizer per Pekar: `CountVectorizer(analyzer=callable)` where the callable runs spaCy POS-aware tokenization, filters to NOUN / ADJ / PROPN within each sentence, and emits 1–3 grams over the filtered per-sentence token list. **Use `analyzer=`, NOT `tokenizer=`** (verified by pre-flight 1, 2026-05-28). The `tokenizer=` pathway lets sklearn's `_word_ngrams` form n-grams over the flat token stream with `ngram_range=(1,3)`, which crosses sentence boundaries and defeats the entire point of the custom vectorizer. The `analyzer=` callable returns the final n-gram list directly; sklearn's `ngram_range` is bypassed.
...
- **R1 — Pekar custom vectorizer integration is more subtle than the summary suggests.** *Resolved 2026-05-28 by pre-flight 1:* the cleanest implementation uses `CountVectorizer(analyzer=callable)` where the callable handles POS-filtering AND n-gram emission within sentence boundaries — this subsumes both Pekar's original `tokenizer=` parameter and the `_word_ngrams` override into a single hook. See Unit 3c implementation note. Remaining mitigation: if MMR diversity on real Coyote pages is unacceptable (R2), fall back to KeyBERT default vectorizer with stricter post-extraction filter (Unit 6).
