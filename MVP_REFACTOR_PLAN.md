...
- `select_mapping_entities(entities, floor=ENTITY_MAP_MENTION_FLOOR, cap=ENTITY_MAP_CAP) -> List[str]` — case-folded mention `Counter`; keep distinct entity strings with count ≥ floor; **then, if `cap is not None`, keep only the top-`cap` survivors by mention count descending**, tie-broken by case-folded ascending alpha (deterministic — same page → same mapping set every run, preserving Coyote's determinism story). Return a stable-ordered list (count desc, then alpha) so truncation is reproducible. **The cap is built in now but DISABLED by default** (`cap=None` ⇒ floor-only behavior, identical to today's spec); PF-9b enables it by setting `ENTITY_MAP_CAP` — a config change, no Unit 6 code reopened (per Gate 7.4's tail-driven-overshoot lever).
